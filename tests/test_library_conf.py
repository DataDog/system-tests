...
class Test_HeaderTags_DynamicConfig:
    ...
    def test_tracing_client_http_header_tags_apm_multiconfig(self):
        ...
        # Validate the root span generated by the first request. The header tags are applied to the HTTP
        # request span, which is the local root span; frameworks that emit several spans sharing the same
        # resource (e.g. rack + sinatra) carry the tags on the root span only.
        s = interfaces.library.get_root_span(request=self.req1)
        # Header tags set via remote config
        assert s["meta"].get("test_header_rc")
        assert s["meta"].get("test_header_rc2")
        assert s["meta"].get("http.request.headers.content-length")
        # Does not have headers set via environment variables
        assert TAG_SHORT not in s["meta"]

        # Validate the root span generated by the second request
        s = interfaces.library.get_root_span(request=self.req2)
        ...
        # Validate the root span generated by the third request. This should be identical to the first request,
        # because we deleted the config with the weblog service and env.
        s = interfaces.library.get_root_span(request=self.req3)
        # Header tags set via remote config
        assert s["meta"].get("test_header_rc")
        assert s["meta"].get("test_header_rc2")
        assert s["meta"].get("http.request.headers.content-length")
