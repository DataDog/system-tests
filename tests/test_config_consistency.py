...
class Test_Config_ObfuscationQueryStringRegexp_Empty:
    ...
    def test_query_string_obfuscation_empty_client(self):
        ...
        client_span = _get_span_by_tags(spans, tags={"span.kind": "client", "http.url": "http://weblog:7777/?key=monkey"})
...
class Test_Config_ObfuscationQueryStringRegexp_Configured:
    ...
    @missing_feature(context.library == "nodejs", reason="Node only obfuscates queries on the server side")
    @missing_feature(context.library < "golang@1.72.0-dev", reason="Client query string collection disabled by default; oObfuscation only occurs on server side")
    @missing_feature(
...
class Test_Config_ObfuscationQueryStringRegexp_Default:
    ...
    @missing_feature(context.library == "nodejs", reason="Node only obfuscates queries on the server side")
    @missing_feature(context.library < "golang@1.72.0-dev", reason="Client query string collection disabled by default; obfuscation only occurs on server side")
    @missing_feature(
        context.library == "java" and context.weblog_variant in ("vertx3", "vertx4"),
        reason="Missing endpoint",
    )
