...
class Test_StandardTagsMethod:
    ...
    def test_methods(self):
        for verb, request in self.requests.items():
            interfaces.library.add_span_tag_validation(
                request=request, tags={"http.method": verb}
            )
    ...
    @bug(
        weblog_variant="spring-boot-payara",
        reason="This weblog variant is currently not accepting TRACE",
    )
    def test_method_trace(self):
        interfaces.library.add_span_tag_validation(
            request=self.trace_request, tags={"http.method": "TRACE"}
        )
...
class Test_StandardTagsUrl:
    ...
    def setup_url_with_sensitive_query_string_legacy(self):
        ...
        self.requests_sensitive_query_string = [
            (
                weblog.get(
                    "/waf?pass=03cb9f67-dbbc-4cb8-b966-329951e10934&key2=val2&key3=val3"
                ),
                r"^.*/waf\?<redacted>&key2=val2&key3=val3$",
            ),
            (
                weblog.get(
                    "/waf?key1=val1&public_key=MDNjYjlmNjctZGJiYy00Y2I4LWI5NjYtMzI5OTUxZTEwOTM0&key3=val3"
                ),
                r"^.*/waf\?key1=val1&<redacted>&key3=val3$",
            ),
            (
                weblog.get(
                    "/waf?key1=val1&key2=val2&token=03cb9f67dbbc4cb8b966329951e10934"
                ),
    ...
    @irrelevant(
        context.library >= "java@1.21.0",
        reason="java released the new version at 1.21.0",
    )
    @irrelevant(
        context.library >= "python@1.18.0rc1",
        reason="python released the new version at 1.19.0",
    )
    @irrelevant(
        context.library >= "dotnet@2.41",
        reason="dotnet released the new version at 2.41.0",
    )
    @irrelevant(
        context.library >= "php@0.93.0", reason="php released the new version at 0.93.0"
    )
    ...
    @irrelevant(
        context.library < "java@1.22.0", reason="java release the new version at 1.22.0"
    )
    @irrelevant(
        context.library < "php@0.93.0", reason="php released the new version at 0.93.0"
    )
    ...
    def test_multiple_matching_substring_legacy(self):
        ...
        interfaces.library.add_span_tag_validation(
            self.request_multiple_matching_substring,
            tags={"http.url": tag},
            value_as_regular_expression=True,
        )
...
class Test_StandardTagsUserAgent:
    ...
    def test_useragent(self):
        ...
        interfaces.library.add_span_tag_validation(
            self.r, tags=tags, value_as_regular_expression=True
        )
...
class Test_StandardTagsStatusCode:
    ...
    def test_status_code(self):
        for code, r in self.requests.items():
            interfaces.library.add_span_tag_validation(
                request=r, tags={"http.status_code": code}
            )
...
class Test_StandardTagsClientIp:
    ...
    def _setup_without_attack(self):
        ...
        for header, value in (
            self.FORWARD_HEADERS | self.FORWARD_HEADERS_VENDOR
        ).items():
            self.requests_without_attack[header] = weblog.get(
                "/waf/", headers={header: value}
            )
    ...
    def _test_client_ip(self, forward_headers):
        for header, _ in forward_headers.items():
            ...
            assert (
                meta["http.client_ip"] in self.VALID_PUBLIC_IPS
            ), f"Unexpected http.client_ip for {header}"
    ...
    @bug(
        library="python",
        reason="cf-connecting-ipv6 seems to have higher precedence than it should",
    )
    @bug(
    ...
    @bug(
        context.library < "java@1.11.0",
        reason="not supported, see https://github.com/DataDog/dd-trace-java/pull/4878",
    )
    ...
    @missing_feature(
        ...
        reason="missing fastly-client-ip, cf-connecting-ip, cf-connecting-ipv6",
    )
    @missing_feature(
        library="golang",
        reason="missing fastly-client-ip, cf-connecting-ip, cf-connecting-ipv6",
