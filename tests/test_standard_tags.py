...
class Test_StandardTagsMethod:
    ...
    @bug(
        weblog_variant="spring-boot-payara", reason="This weblog variant is currently not accepting TRACE",
    )
...
class Test_StandardTagsUrl:
    ...
    @irrelevant(
        context.library >= "java@1.21.0", reason="java released the new version at 1.21.0",
    )
    @irrelevant(
        context.library >= "python@1.18.0rc1", reason="python released the new version at 1.19.0",
    )
    @irrelevant(
        context.library >= "dotnet@2.41", reason="dotnet released the new version at 2.41.0",
    )
...
class Test_StandardTagsClientIp:
    ...
    @bug(
        library="python", reason="cf-connecting-ipv6 seems to have higher precedence than it should",
    )
    ...
    @missing_feature(
        library="golang", reason="missing fastly-client-ip, cf-connecting-ip, cf-connecting-ipv6",
    )
    @missing_feature(
        library="nodejs", reason="missing fastly-client-ip, cf-connecting-ip, cf-connecting-ipv6",
    )
    @missing_feature(
        library="ruby", reason="missing fastly-client-ip, cf-connecting-ip, cf-connecting-ipv6",
    )
    @missing_feature(
        library="php", reason="missing fastly-client-ip, cf-connecting-ip, cf-connecting-ipv6",
    )
