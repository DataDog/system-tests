...
class _BaseOtelDbIntegrationTestClass(BaseDbIntegrationsTestClass):
    ...
    @bug(
        library="python_otel", reason="Open Telemetry doesn't send this span for python but it should do",
    )
    @bug(
        library="nodejs_otel", reason="Open Telemetry doesn't send this span for nodejs but it should do",
    )
    ...
    @missing_feature(
        library="nodejs_otel", reason="Open telemetry with nodejs is not generating this information.",
    )
