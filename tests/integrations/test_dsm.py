...
class Test_DsmSQS:
    ...
    def setup_dsm_sqs(self):
        ...
        if context.library.library != "nodejs":
            self.queue = f"{DSM_QUEUE}_{context.library.library}_{WEBLOG_VARIANT_SANITIZED}"
        else:
...
class Test_DsmSNS:
    ...
    def setup_dsm_sns(self):
        ...
        # which changes for each run with the time stamp added
        if context.library.library != "nodejs":
            self.topic = f"{DSM_TOPIC}_{context.library.library}_{WEBLOG_VARIANT_SANITIZED}_raw"
            self.queue = f"{DSM_QUEUE_SNS}_{context.library.library}_{WEBLOG_VARIANT_SANITIZED}_raw"
        else:
...
class Test_DsmKinesis:
    ...
    def setup_dsm_kinesis(self):
        ...
        # which changes for each run with the time stamp added
        if context.library.library != "nodejs":
            self.stream = f"{DSM_STREAM}_{context.library.library}_{WEBLOG_VARIANT_SANITIZED}"
        else:
