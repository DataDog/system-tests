...
class _BaseDatadogDbIntegrationTestClass(BaseDbIntegrationsTestClass):
    ...
    def get_spans(self, excluded_operations=(), operations=None):
        ...
        for db_operation, request in self.get_requests(
            excluded_operations, operations=operations
        ):
    ...
    def test_db_type(self, excluded_operations=()):
        ...
        for db_operation, span in self.get_spans(
            ...
            assert (
                span["meta"]["db.type"] == self.db_service
    ...
    def test_db_name(self):
        ...
        for db_operation, span in self.get_spans():
            assert (
                span["meta"]["db.name"] == db_container.db_instance
            ), f"Test is failing for {db_operation}"
    ...
    def test_db_system(self):
        ...
        for db_operation, span in self.get_spans():
            assert (
                span["meta"]["db.system"] == self.db_service
            ), f"Test is failing for {db_operation}"
    ...
    def test_NOT_obfuscate_query(self):
        ...
        for db_operation, request in self.get_requests():
            ...
            assert (
                span["resource"].count("?") == 0
            ), f"The query should not be obfuscated for operation {db_operation}"
...
class Test_MySql(_BaseDatadogDbIntegrationTestClass):
    ...
    @bug(
        library="python",
        reason="the value of this span should be 'world' instead of  'b'world'' ",
    )
    def test_db_name(self):
        super().test_db_name()
    ...
    @bug(
        library="python",
        reason="the value of this span should be 'mysqldb' instead of  'b'mysqldb'' ",
    )
    def test_db_user(self, excluded_operations=()):
        super().test_db_user()
    ...
    def test_obfuscate_query(self, excluded_operations=()):
        ...
        for db_operation, request in self.get_requests(
            ...
            if db_operation == "insert":
                ...
                expected_obfuscation_count = (
                    0 if context.library.library == "nodejs" else 2
                )
            else:
