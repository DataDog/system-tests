...
@scenarios.graphql_appsec_exception
@features.graphql_query_error_reporting
class Test_GraphQLQueryErrorReportingException:
    """Test if GraphQL query errors create span events with Error Tracking RFC specification"""
    ...
    def test_execute_error_span_event(self):
        """Test if the main GraphQL span contains a span event with the appropriate error information
        following the Error Tracking RFC specification.
        The /graphql endpoint must support the query `query myQuery { withError }` which will return an
        error response with the following structure:
        {
            "errors": [
        ...
        This test validates the Error Tracking RFC specification where:
        - Span event name is "exception" instead of "dd.graphql.query.error"
        - Attributes are renamed to follow OpenTelemetry semantics:
          - message -> exception.message
          - type -> exception.type
          - stacktrace -> exception.stacktrace
          - locations -> graphql.error.location
          - path -> graphql.error.path
        ...
        # Check OpenTelemetry exception attributes
        assert isinstance(attributes["exception.message"], str)
        assert isinstance(attributes["exception.type"], str)
        assert isinstance(attributes["exception.stacktrace"], str)

        # Check GraphQL-specific attributes
        for path in attributes["graphql.error.path"]:
            assert isinstance(path, str)

        if self._has_location(span):
            location = attributes["graphql.error.location"]
            assert len(location) == 1
        ...
        # Check extensions (these should be namespaced under graphql.error.extensions)
        extensions = attributes["graphql.error.extensions"]
        assert extensions["int"] == 1
        assert extensions["float"] == 1.1
        assert extensions["str"] == "1"
        assert extensions["bool"] is True
        ...
        # The goal here is to display the original data with as much fidelity as possible, without allowing
        # for arbitrary nested levels inside `span_event.attributes`.
        assert "1" in extensions["other"]
        assert "foo" in extensions["other"]

        assert "not_captured" not in extensions
    ...
    def _get_events(span) -> dict:
        if "events" in span["meta"]:
            ...
            for event in events:
                ...
                for key, value in attributes.items():
                    attributes[key] = Test_GraphQLQueryErrorReportingException._parse_event_value(value)
    ...
    def _parse_event_value(value) -> int | str | bool | float | list[Any]:
        ...
        if type_ == 0:
            ...
            return [Test_GraphQLQueryErrorReportingException._parse_event_value(v) for v in value["array_value"]["values"]]
        else:
            raise ValueError(f"Unsupported span event attribute type {type_} for: {value}")
