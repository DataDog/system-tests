...
def get_keyvalue_generator(attributes: list[dict]) -> Iterator[tuple[str, Any]]:
    for keyValue in attributes:
        if keyValue["value"].get("stringValue"):
...
class Test_Otel_Tracing_OTLP:
    ...
    def test_tracing(self):
        ...
        attributes = {keyValue["key"]: keyValue["value"]["stringValue"] for keyValue in resource_span.get("resource").get("attributes")}
        ...
        assert span.get("startTimeUnixNano")
        assert span.get("endTimeUnixNano")
        ...
        # attributes = {keyValue["key"]: keyValue["value"]["stringValue"] for keyValue in span.get("attributes")}
