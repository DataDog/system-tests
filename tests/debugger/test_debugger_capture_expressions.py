    ...
    def _validate_capture_expressions(self, expected_captures: dict[str, dict]):
        ...
        for probe_id in self.probe_ids:
            ...
            for snapshot in snapshots:
                ...
                # For capture expressions, they are nested in captures.return.captureExpressions (or captures.entry.captureExpressions)
                # We check return first, then fall back to entry
                ...
                if capture_expressions is None:
                    raise ValueError(
                        ...
                        f"in captures.return or captures.entry"
                    )
...
@missing_feature(context.library == "python", reason="Not yet implemented", force_skip=True)
@missing_feature(
    context.library == "java", reason="Line probes with capture expressions not yet implemented", force_skip=True
)
