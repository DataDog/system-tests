...
def assert_expected_telemetry_route(matcher: Callable[[JSON], bool], description: str) -> None:
    ...
    assert not unexpected.wait_for(lambda data: matcher(cast("JSON", data)), timeout=UNEXPECTED_ROUTE_WAIT_SECONDS), (
        f"{description} was duplicated through the non-selected telemetry route"
    )
