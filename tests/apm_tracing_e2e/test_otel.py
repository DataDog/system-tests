...
class Test_Otel_Span:
    ...
    @bug(
        context.library == "java", reason="Span.kind is not set to internal, have type instead",
    )
    ...
    @irrelevant(
        condition=context.library != "golang", reason="Golang specific test with OTel Go contrib package",
    )
