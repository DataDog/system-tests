...
@bug(
    context.library in ("nodejs@3.2.0", "nodejs@2.15.0"),
...
class Test_Main:
    ...
    def setup_main(self):
        ...
        for i in range(10):
            for _ in range(5):
                self.requests.append(
                    weblog.get("/waf/", headers={"User-Agent": "Arachni/v1"})
                )
        ...
        logger.debug(
            f"Sent 50 requests in {(datetime.datetime.now() - start_time).total_seconds()} s"
        )
