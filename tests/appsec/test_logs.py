...
class Test_Standardization:
    ...
    @irrelevant(
        library="java", reason="Cannot be implemented with cooperation from libddwaf"
    )
    @missing_feature(library="php")
    def test_d01(self):
        """Log D1: names and adresses AppSec listen to"""
        stdout.assert_presence(
            r"Loaded rule:", level="DEBUG"
    ...
    def test_d02(self):
        ...
        stdout.assert_presence(
            r"Pushing address .* to the Instrumentation Gateway", level="DEBUG"
        )
    ...
    def test_d03(self):
        ...
        stdout.assert_presence(
            r"Available addresses .* match needs for rules", level="DEBUG"
        )
    ...
    def test_d04(self):
        ...
        stdout.assert_presence(
            r"Executing AppSec In-App WAF with parameters:", level="DEBUG"
        )
    ...
    def test_d05(self):
        """Log D5: WAF outputs"""
        stdout.assert_presence(
            r'AppSec In-App WAF returned:.*crs-921-160"', level="DEBUG"
        )
        stdout.assert_presence(
            r'AppSec In-App WAF returned:.*crs-913-110"', level="DEBUG"
        )
    ...
    def test_d06(self):
        ...
        stdout.assert_presence(
            r"Detecting an attack from rule crs-921-160:.*", level="DEBUG"
        )
        stdout.assert_presence(
            r"Detecting an attack from rule crs-913-110:.*", level="DEBUG"
        )
    ...
    def test_i01(self):
        ...
        stdout.assert_presence(
            r"AppSec initial configuration from .*, libddwaf version: \d+\.\d+\.\d+",
            level="INFO",
        )

    @missing_feature(
        library="php", reason="rules are not analyzed, only converted to PWArgs"
    )
    @missing_feature(library="dotnet", reason="APPSEC-983")
    def test_i02(self):
    ...
    def test_i05(self):
        ...
        stdout.assert_presence(
            r"Detecting an attack from rule crs-921-160$", level="INFO"
        )
        stdout.assert_presence(
            r"Detecting an attack from rule crs-913-110$", level="INFO"
        )
...
class Test_StandardizationBlockMode:
    ...
    def test_i06(self):
        ...
        stdout.assert_presence(
            r"Blocking current transaction \(rule: .*\)$", level="INFO"
        )
