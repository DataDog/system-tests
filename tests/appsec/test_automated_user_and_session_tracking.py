...
class Test_Automated_User_Tracking:
    def setup_user_tracking(self):
        self.r_login = weblog.post("login?auth=local", data=login_data(context))
        self.cookies = self.r_login.cookies
        self.r_user = weblog.get("user/me", cookies=self.cookies,)

    def test_user_tracking(self):
        assert self.r_login.status_code == 200
        assert self.r_user.status_code == 200

        for _, trace, span in interfaces.library.get_spans(request=self.r_user):
            meta = span.get("meta", {})
    ...
    def setup_user_tracking_sdk_overwrite(self):
        self.r_login = weblog.post("login?auth=local", data=login_data(context))
        self.cookies = self.r_login.cookies
        self.r_user = weblog.get("user/me?sdk_user=sdkUser", cookies=self.cookies,)
...
class Test_Automated_User_Blocking:
    ...
    def test_user_blocking(self):
        ...
        interfaces.library.assert_waf_attack(self.r_user_blocked, rule="block-users")
        assert self.r_user_blocked.status_code == 403
...
class Test_Automated_Session_Blocking:
    ...
    def test_session_blocking(self):
        ...
        assert self.config_state_2[rc.RC_STATE] == rc.ApplyState.ACKNOWLEDGED
        interfaces.library.assert_waf_attack(self.r_session_blocked, rule="block-sessions")
        assert self.r_session_blocked.status_code == 403
