...
class TestRequestParameterName:
    ...
    def test_parametername(self):
        weblog.post(
            "/iast/source/parametername/test", data={"source": "parameterName", "value": "parameterNameValue"}
        )
        interfaces.library_stdout.wait()
        testSource = re.escape('tainted={"value":"source","ranges":[{"source":{"origin":"http.request.parameter.name","name":"source"},"start":0,"length":6}]}')
        interfaces.library_stdout.assert_presence(testSource, level="DEBUG")
        testValue = re.escape('tainted={"value":"value","ranges":[{"source":{"origin":"http.request.parameter.name","name":"value"},"start":0,"length":5}]}')
