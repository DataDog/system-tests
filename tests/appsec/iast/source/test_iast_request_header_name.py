...
class TestRequestHeaderName:
    ...
    def test_headername(self):
        weblog.get(
            "/iast/source/headername/test", headers={"random-key": "header-name"}
        )
        interfaces.library_stdout.wait()
        test = re.escape('tainted={"value":"random-key","ranges":[{"source":{"origin":"http.request.header.name","name":"random-key"},"start":0,"length":10}]}')
