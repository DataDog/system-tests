...
class Test_Headers:
    ...
    @irrelevant(
        library="php", reason="PHP normalizes into dashes; additionally, matching on keys is not supported",
    )
...
class Test_Cookies:
    ...
    @irrelevant(
        context.appsec_rules_version >= "1.2.7", reason="cookies were disabled for the time being",
    )
    ...
    @irrelevant(
        library="dotnet", reason="One space in the whole value cause kestrel to erase the whole value",
    )
    @irrelevant(
        context.appsec_rules_version >= "1.2.7", reason="cookies were disabled for the time being",
    )
    ...
    @irrelevant(library="golang", reason="not handled by the Go standard cookie parser")
    @irrelevant(
        library="dotnet", reason="Quotation marks cause kestrel to erase the whole value",
    )
...
@coverage.basic
@bug(
    context.library == "nodejs@2.8.0", reason="Capability to read body content is broken",
)
@features.appsec_request_blocking
