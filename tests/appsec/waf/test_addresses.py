...
class Test_UrlQueryKey:
    ...
    def test_query_key(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r, pattern="$eq", address="server.request.query"
        )
...
class Test_UrlQuery:
    ...
    def setup_query_argument(self):
        self.r_query_argument = weblog.get(
            "/waf/", params={"attack": "appscan_fingerprint"}
        )

    def test_query_argument(self):
        """AppSec catches attacks in URL query value"""
        interfaces.library.assert_waf_attack(
            self.r_query_argument,
            pattern="appscan_fingerprint",
    ...
    def test_query_encoded(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r_query_encoded, address="server.request.query"
        )

    def setup_query_with_strict_regex(self):
        self.r_query_with_strict_regex = weblog.get(
            "/waf/", params={"value": "0000012345"}
        )
    ...
    def test_query_with_strict_regex(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r_query_with_strict_regex,
            pattern="0000012345",
            address="server.request.query",
        )
...
class Test_UrlRaw:
    ...
    def test_path(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r, pattern="0x5c0x2e0x2e0x2f", address="server.request.uri.raw"
        )
...
class Test_Headers:
    ...
    @irrelevant(
        library="php",
        reason="PHP normalizes into dashes; additionally, matching on keys is not supported",
    )
    @missing_feature(
    ...
    def test_specific_key3(self):
        ...
        ADDRESS = "server.request.headers.no_cookies"
        interfaces.library.assert_waf_attack(
            self.r_sk_5, address=ADDRESS, key_path=["referer"]
        )
        interfaces.library.assert_waf_attack(
            self.r_sk_6, address=ADDRESS, key_path=["referer"]
        )
...
class Test_Cookies:
    ...
    @irrelevant(
        context.appsec_rules_version >= "1.2.7",
        reason="cookies were disabled for the time being",
    )
    def test_cookies(self):
        """Appsec WAF detects attackes in cookies"""
        interfaces.library.assert_waf_attack(
            self.r_cookies, pattern=".htaccess", address="server.request.cookies"
        )
    ...
    def test_cookies_with_semicolon(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r_cwsc_1, pattern=";shutdown--", address="server.request.cookies"
        )
        interfaces.library.assert_waf_attack(
    ...
    @irrelevant(
        library="dotnet",
        reason="One space in the whole value cause kestrel to erase the whole value",
    )
    @irrelevant(
        context.appsec_rules_version >= "1.2.7",
        reason="cookies were disabled for the time being",
    ...
    def test_cookies_with_spaces(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r_cws, pattern="var_dump ()", address="server.request.cookies"
        )
    ...
    @irrelevant(
        library="dotnet",
        reason="Quotation marks cause kestrel to erase the whole value",
    )
    ...
    def test_cookies_with_special_chars2(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r_cwsc2, pattern='o:4:"x":5:{d}', address="server.request.cookies"
        )
    ...
    def test_cookies_custom_rules(self):
        """Appsec WAF detects attackes in cookies"""
        interfaces.library.assert_waf_attack(
            self.r_ccr, pattern=".htaccess", address="server.request.cookies"
        )
    ...
    @irrelevant(library="golang", reason="Not handled by the Go standard cookie parser")
    @irrelevant(
        library="python", reason="Not handled by the Python standard cookie parser"
    )
    @scenarios.appsec_custom_rules
    def test_cookies_with_semicolon_custom_rules(self):
        """Cookie with pattern containing a semicolon"""
        interfaces.library.assert_waf_attack(
            self.r_cwsccr, pattern=";shutdown--", address="server.request.cookies"
        )
    ...
    def test_cookies_with_spaces_custom_rules(self):
        """Cookie with pattern containing a space"""
        interfaces.library.assert_waf_attack(
            self.r_cwscr_2, pattern="var_dump ()", address="server.request.cookies"
        )
    ...
    def test_cookies_with_special_chars2_custom_rules(self):
        """Other cookies patterns"""
        interfaces.library.assert_waf_attack(
            self.r_cwsc2cc, pattern='o:4:"x":5:{d}', address="server.request.cookies"
        )
...
@coverage.basic
@bug(
    context.library == "nodejs@2.8.0",
    reason="Capability to read body content is broken",
)
@features.appsec_request_blocking
class Test_BodyUrlEncoded:
    ...
    def test_body_value(self):
        """AppSec detects attacks in URL encoded body values"""
        interfaces.library.assert_waf_attack(
            self.r_value, value='<vmlframe src="xss">', address="server.request.body"
        )
...
class Test_BodyJson:
    ...
    @irrelevant(
        reason="unsupported by framework", library="ruby", weblog_variant="rack"
    )
    @irrelevant(
        reason="unsupported by framework", library="ruby", weblog_variant="sinatra14"
    )
    @irrelevant(
        reason="unsupported by framework", library="ruby", weblog_variant="sinatra20"
    )
    @irrelevant(
        reason="unsupported by framework", library="ruby", weblog_variant="sinatra21"
    )
    @irrelevant(
        reason="unsupported by framework", library="ruby", weblog_variant="uds-sinatra"
    )
    def test_json_array(self):
        """AppSec detects attacks in JSON body arrays"""
        interfaces.library.assert_waf_attack(
            self.r_array, value='<vmlframe src="xss">', address="server.request.body"
        )
...
class Test_BodyXml:
    ...
    def setup_xml_attr_value(self):
        ...
        self.r_attr_2 = self.weblog_post(
            "/waf", data=f'<string attack="{self.ENCODED_ATTACK}" />'
        )

    @bug(context.weblog_variant == "spring-boot-wildfly")
    def test_xml_attr_value(self):
        interfaces.library.assert_waf_attack(
            self.r_attr_1, address="server.request.body", value="var_dump ()"
        )
        interfaces.library.assert_waf_attack(
            self.r_attr_2, address="server.request.body", value=self.ATTACK
        )

    def setup_xml_content(self):
        self.r_content_1 = self.weblog_post("/waf", data="<string>var_dump ()</string>")
        self.r_content_2 = self.weblog_post(
            "/waf", data=f"<string>{self.ENCODED_ATTACK}</string>"
        )

    @bug(context.weblog_variant == "spring-boot-wildfly")
    def test_xml_content(self):
        interfaces.library.assert_waf_attack(
            self.r_content_1, address="server.request.body", value="var_dump ()"
        )
        interfaces.library.assert_waf_attack(
            self.r_content_2, address="server.request.body", value=self.ATTACK
        )
...
class Test_ResponseStatus:
    ...
    def test_basic(self):
        ...
        interfaces.library.assert_waf_attack(
            self.r, pattern="404", address="server.response.status"
        )
...
class Test_gRPC:
    ...
    def setup_basic(self):
        self.requests = [
            weblog.grpc('" OR TRUE --'),
            weblog.grpc(
                "SELECT * FROM users WHERE name='com.sun.org.apache' UNION SELECT creditcard FROM users"
            ),
            weblog.grpc(
                "SELECT * FROM users WHERE id=1 UNION SELECT creditcard FROM users"
            ),
        ]
    ...
    def test_basic(self):
        ...
        for r in self.requests:
            interfaces.library.assert_waf_attack(
                r, address="grpc.server.request.message"
            )


@rfc(
    "https://datadoghq.atlassian.net/wiki/spaces/APS/pages/2278064284/gRPC+Protocol+Support"
)
@features.appsec_request_blocking
@features.grpc_threats_management
class Test_FullGrpc:
