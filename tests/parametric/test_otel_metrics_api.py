...
class Test_Otel_Metrics_Api:
    ...
    def test_otel_asynchronous_counter_constant_callback_value(self, test_agent, test_library, n):
        ...
        with test_library as t:
            t.disable_traces_flush()
            t.otel_get_meter(name=meter_name, version="1.0.0", schema_url="https://opentelemetry.io/schemas/1.27.0", attributes=expected_scope_attributes)
            t.otel_create_asynchronous_counter(meter_name, name, unit, description, n, expected_add_attributes)
            time.sleep(0.5)
    ...
    def test_otel_asynchronous_updowncounter_constant_callback_value(self, test_agent, test_library, n):
        ...
        with test_library as t:
            t.disable_traces_flush()
            t.otel_get_meter(name=meter_name, version="1.0.0", schema_url="https://opentelemetry.io/schemas/1.27.0", attributes=expected_scope_attributes)
            t.otel_create_asynchronous_updowncounter(meter_name, name, unit, description, n, expected_add_attributes)
            time.sleep(0.5)
