...
class Test_128_Bit_Traceids:
    ...
    def test_datadog_128_bit_propagation_tid_long(self, test_agent, test_library):
        ...
        assert (
            "x-datadog-tags" not in headers
            or "_dd.p.tid=" not in headers["x-datadog-tags"]
        )
    ...
    @missing_feature(
        context.library == "dotnet", reason="Optional feature not implemented"
    )
    @missing_feature(
        context.library == "golang", reason="Optional feature not implemented"
    )
    @missing_feature(context.library == "nodejs", reason="not implemented")
    @missing_feature(context.library == "ruby", reason="not implemented")
    @pytest.mark.parametrize(
    ...
    def test_datadog_128_bit_propagation_tid_malformed_optional_tag(
        ...
        assert (
            get_span(test_agent)["meta"].get("_dd.propagation_error")
            == "malformed_tid XXXX"
        )
    ...
    def test_datadog_128_bit_generation_enabled_by_default(
        self, test_agent, test_library
    ):
        """Ensure that 128-bit TraceIds are properly generated, propagated in
        datadog headers, and populated in trace data.
    ...
    def test_b3single_128_bit_propagation_and_generation(
        self, test_agent, test_library
    ):
        """Ensure that a new span from incoming headers does not modify the trace id when generation is true.
        """
    ...
    @missing_feature(
        ...
        reason="Issue: Ruby doesn't support case-insensitive distributed headers",
    )
    @pytest.mark.parametrize(
        "library_env",
        [
    ...
    def test_b3single_128_bit_generation_disabled(self, test_agent, test_library):
        ...
        check_64_bit_trace_id(
            fields[0], span.get("trace_id"), span["meta"].get("_dd.p.tid")
        )
    ...
    def test_w3c_128_bit_propagation_tid_in_parent(self, test_agent, test_library):
        ...
        with test_library:
            with test_library.start_span(
                name="parent", service="service", resource="resource"
            ) as parent:
                with test_library.start_span(
                    name="child", service="service", parent_id=parent.span_id
                ):
                    pass
    ...
    @missing_feature(
        ...
        reason="inconsistent_tid is not implemented for w3c",
    )
    @missing_feature(context.library == "ruby", reason="not implemented")
    @pytest.mark.parametrize(
        "library_env",
    ...
    def test_w3c_128_bit_propagation_tid_inconsistent_optional_tag(
        self, test_agent, test_library
    ):
        """Ensure that if the trace state contains a tid that is inconsistent with the trace id from
    ...
    @missing_feature(context.library == "nodejs", reason="not implemented")
    @missing_feature(
        context.library == "python", reason="malformed_tid is not implemented"
    )
    @missing_feature(context.library == "ruby", reason="not implemented")
    @pytest.mark.parametrize(
        "library_env",
    ...
    def test_w3c_128_bit_propagation_tid_malformed_optional_tag(
        self, test_agent, test_library
    ):
        ...
        with test_library:
            headers = make_single_request_and_get_inject_headers(
                ...
                [
                    [
                        "traceparent",
                        "00-0000000000000000abcdefab12345678-000000003ade68b1-01",
                    ],
                ],
            )
        span = get_span(test_agent)
    ...
    def test_w3c_128_bit_generation_disabled(self, test_agent, test_library):
        ...
        check_64_bit_trace_id(
            fields[1], span.get("trace_id"), span["meta"].get("_dd.p.tid")
        )
