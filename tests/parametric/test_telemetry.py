...
@scenarios.parametric
@features.telemetry_app_started_event
class Test_ConfigTelemetryCompleteness:
    """
    ...
    def test_library_settings(self, test_agent, test_library):
        config_norm_rules = self.load_telemetry_json("config_norm_rules")
        config_prefix_block_list = self.load_telemetry_json("config_prefix_block_list")
        config_aggregation_list = self.load_telemetry_json("config_aggregation_list")

        lang_configs = {}
        for lang in ["dotnet", "go", "jvm", "nodejs", "php", "python", "ruby"]:
            lang_configs[lang] = self.load_telemetry_json(lang + "_config_rules")

        lang_config = lang_configs.get(context.library) or {}
