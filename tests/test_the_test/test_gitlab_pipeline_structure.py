...
def test_parametric_jobs_limit_xdist_workers_and_preserve_throughput(tmp_path: Path) -> None:
    ...
    params = {
        ...
        "parametric": {"enable": True, "job_count": job_count, "job_matrix": list(range(1, job_count + 1))},
    }
    ...
    assert all(job["variables"]["PYTEST_XDIST_AUTO_NUM_WORKERS"] == str(worker_count) for job in jobs)
