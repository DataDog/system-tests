...
class TestOnboardingInstall:
    def test_forTraces(self, onboardig_vm_ip):
        ...
        logger.info(f"Launching test for : [{onboardig_vm_ip}]")
        logger.info(f"Waiting for weblog available [{onboardig_vm_ip}]")
        wait_for_port(5985, onboardig_vm_ip, 60.0)
        logger.info(f"[{onboardig_vm_ip}]: Weblog app is ready!")
        logger.info(f"Making a request to weblog [{onboardig_vm_ip}]")
        request_uuid = make_get_request("http://" + onboardig_vm_ip + ":5985/")
        logger.info(f"Http request done with uuid: [{request_uuid}] for ip [{onboardig_vm_ip}]")
