...
def save_aws_ssi_json(aws_ssi_data: dict) -> None:
    ...
    try:
        ...
        backup_path = AWS_SSI_JSON_PATH.with_suffix(AWS_SSI_JSON_PATH.suffix + ".bak")
        with open(str(AWS_SSI_JSON_PATH), "r") as src, open(str(backup_path), "w") as backup:
            backup.write(src.read())

        # Write the updated data
        with open(str(AWS_SSI_JSON_PATH), "w") as file:
...
def main() -> None:
    ...
    def select_multiple_items(items: list[Any], prompt_text: str) -> list[Any]:
    ...
    if has_incompatible:
        print(
            f"\n{Colors.YELLOW}Do you want to make any incompatible weblog compatible with "
            f"{vm_name}? (y/n){Colors.ENDC}"
        )
        choice = input().lower()
        ...
        if choice == "y":
            ...
            available_languages = [lang for lang in incompatible_weblogs if incompatible_weblogs[lang]]
                ...
                if 0 <= lang_index < len(available_languages):
                    ...
                    if selected_weblogs:
                        for weblog in selected_weblogs:
                            print(
                                f"{Colors.YELLOW}Making {weblog} ({language_choice}) "
                                f"compatible with {vm_name}...{Colors.ENDC}"
                            )
                            aws_ssi_data = update_aws_ssi_json(
                                aws_ssi_data, vm_details, weblog, language_choice, make_compatible=True
                            )
                            changes_made = True
                    else:
    ...
    if has_compatible:
        print(
            f"\n{Colors.YELLOW}Do you want to make any compatible weblog incompatible with "
            f"{vm_name}? (y/n){Colors.ENDC}"
        )
        choice = input().lower()
        ...
        if choice == "y":
            ...
            available_languages = [lang for lang in compatible_weblogs if compatible_weblogs[lang]]
            for i, language in enumerate(available_languages, 1):
                print(f"  {i}. {language}")
                ...
                if 0 <= lang_index < len(available_languages):
                    ...
                    if selected_weblogs:
                        for weblog in selected_weblogs:
                            print(
                                f"{Colors.YELLOW}Making {weblog} ({language_choice}) "
                                f"incompatible with {vm_name}...{Colors.ENDC}"
                            )
                            aws_ssi_data = update_aws_ssi_json(
                                aws_ssi_data, vm_details, weblog, language_choice, make_compatible=False
                            )
                            changes_made = True
                    else:
                        print(f"{Colors.BLUE}No weblogs selected for compatibility changes.{Colors.ENDC}")
