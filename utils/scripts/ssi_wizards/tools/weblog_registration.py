...
def update_scenario_matrix(aws_ssi_data: dict, weblog_name: str, language: str, selected_scenarios: list[str]) -> dict:
    ...
    for scenario in selected_scenarios:
        ...
        if not entry_found:
            ...
            scenario_matrix.append({"scenarios": [scenario], "weblogs": [{language: [weblog_name]}]})
...
def update_weblog_spec(
    ...
    # Use empty string as default value for None
    compatible_branches = {vm.get("os_branch", "") for vm in compatible_vms}
    compatible_names = {vm.get("name", "") for vm in compatible_vms}
    compatible_types = {vm.get("os_type", "") for vm in compatible_vms}

    all_branches = {vm.get("os_branch", "") for vm in all_vms}
    all_names = {vm.get("name", "") for vm in all_vms}
    all_types = {vm.get("os_type", "") for vm in all_vms}
    ...
    # If the weblog is compatible with only specific OS branches
        ...
        # Sort with a key function to ensure we can compare all values
        weblog_spec["exact_os_branches"] = sorted(compatible_branches, key=lambda x: str(x))
        weblog_spec.pop("excluded_os_branches", None)
        weblog_spec.pop("excluded_os_names", None)
        weblog_spec.pop("excluded_os_types", None)
    ...
    # Otherwise, specify which VMs are excluded
        ...
        if incompatible_branches:
            weblog_spec["excluded_os_branches"] = sorted(incompatible_branches, key=lambda x: str(x))
        else:
            weblog_spec.pop("excluded_os_branches", None)
        ...
        incompatible_names = {
            ...
            if not any(vm.get("os_branch") in incompatible_branches for vm in all_vms if vm.get("name") == name)
        }
        if incompatible_names:
            weblog_spec["excluded_os_names"] = sorted(incompatible_names, key=lambda x: str(x))
        else:
            weblog_spec.pop("excluded_os_names", None)
        ...
        if incompatible_types:
            weblog_spec["excluded_os_types"] = sorted(incompatible_types, key=lambda x: str(x))
        else:
            weblog_spec.pop("excluded_os_types", None)
...
def print_weblog_info(
    ...
    if scenarios:
        # Use str as key function to handle potential None values
        for scenario in sorted(scenarios, key=lambda x: str(x) if x is not None else ""):
            print(f"  - {scenario}")
    else:
    ...
    if compatible_vms:
        # Use an explicit key function that handles None values
        for vm in sorted(compatible_vms, key=lambda x: str(x.get("name", ""))):
    ...
    if incompatible_vms:
        # Use an explicit key function that handles None values
        for vm in sorted(incompatible_vms, key=lambda x: str(x.get("name", ""))):
            print(f"  - {vm.get('name', '')} ({vm.get('os_branch', '')})")
    else:
...
def main() -> None:
    ...
    selected_scenarios = select_multiple_items(
        all_scenarios, f"Select scenarios to associate with {weblog_name} ({language}):", preselected=existing_scenarios
    )
    ...
    status = "updated" if is_existing_weblog else "registered"
    print(
        f"\n{Colors.GREEN}{Colors.BOLD}Weblog {weblog_name} ({language}) has been successfully {status}.{Colors.ENDC}"
    )
