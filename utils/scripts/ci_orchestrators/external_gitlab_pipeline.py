...
def handle_parallelism(yaml_data: dict) -> None:
    """Keep only ['compute_pipeline'] for jobs that need compute_pipeline plus another
    job, so the per-language pipelines run in parallel in external repositories.
    (In the system-tests repo itself the languages run sequentially.)

    Release-time staggering of the multi-language fan-out used to be generated here
    (delayed_*_trigger jobs). That is now expressed as static YAML in the consuming
    pipeline (see auto_inject's .gitlab/fragments/ssi_orchestration.yml).
    """
