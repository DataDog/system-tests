...
if [[ $CI_COMMIT_MESSAGE =~ ($PR_PATTERN) ]]; then
    ...
    if [ -z "$is_build_buddies" ] && [ -z "$is_build_python_base_images" ] && [ -z "$is_build_lib_injection_app_images" ]; then
    ...
    if [ -z "$is_build_python_base_images" ]; then
    ...
    if [ -z "$is_build_lib_injection_app_images" ]; then
