...
if [[ $CI_COMMIT_MESSAGE =~ ($PR_PATTERN) ]]; then
    ...
    is_build_nodejs_base_images=$(echo "$PR_DATA" | jq -c '.[] | select(.name | contains("build-nodejs-base-images"))');
    ...
    if [ -z "$is_build_buddies" ] && [ -z "$is_build_nodejs_base_images" ] && [ -z "$is_build_python_base_images" ] && [ -z "$is_build_lib_injection_app_images" ]
    then
    ...
    if [ -z "$is_build_nodejs_base_images" ]
    then
        echo "The PR $PR_NUMBER doesn't contain the 'build-nodejs-base-images' label "
    else
        echo "The PR $PR_NUMBER contains the 'build-nodejs-base-images' label. Launching the images generation process "
        ./utils/build/build_nodejs_base_images.sh --push
        echo "------------- The nodejs base images have been built and pushed ------------- "
    fi
