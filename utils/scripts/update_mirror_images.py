...
RefreshImages = set[str] | Literal["all"]
...
def _parse_refresh(value: str | None) -> RefreshImages | None:
    if value is None:
        return None

    images = {image.strip() for image in value.split(",") if image.strip()}
    if not images:
        raise ValueError("--refresh requires at least one image reference or 'all'")

    if "all" in images:
        if len(images) > 1:
            raise ValueError("--refresh 'all' cannot be combined with image references")
        return "all"

    return images


def _load_lock_data() -> dict[str, object]:
    data = yaml.safe_load(LOCK_YAML.read_text(encoding="utf-8")) or {}
    if not isinstance(data, dict):
        raise TypeError(f"{LOCK_YAML} must contain a YAML mapping")
    return data


def _write_lock_data(data: dict[str, object]) -> None:
    header = "# Auto-generated by: uv run bin/mirror_images.py lock\n# Do not edit manually.\n"
    LOCK_YAML.write_text(f"{header}{yaml.safe_dump(data, sort_keys=False)}", encoding="utf-8")


def _refresh_lock_entries(refresh: RefreshImages) -> str | None:
    if not LOCK_YAML.exists():
        print(f"{LOCK_YAML} does not exist; nothing to refresh.", flush=True)
        return None
    ...
    if not isinstance(images, dict):
        raise TypeError(f"{LOCK_YAML} must contain an 'images' mapping")

    if refresh == "all":
        removed_count = len(images)
        if removed_count == 0:
            print(f"{LOCK_YAML} has no image entries to refresh.", flush=True)
            return None
        images.clear()
        print(f"Removed all {removed_count} image lock entries before refresh.", flush=True)
    else:
        removed_images = sorted(image for image in refresh if image in images)
        missing_images = sorted(refresh - set(images))
        if not removed_images:
            print("No existing lock entries matched --refresh.", flush=True)
            return None
        ...
        print(
            f"Removed {len(removed_images)} image lock entries before refresh: {', '.join(removed_images)}", flush=True
        )
        if missing_images:
            print(
                f"Skipped {len(missing_images)} image(s) not present in the lock file: {', '.join(missing_images)}",
                flush=True,
            )

    _write_lock_data(data)
    return original_lock


def main(excluded: set[str], *, skip_lock: bool, refresh: RefreshImages | None = None) -> None:
    if skip_lock and refresh is not None:
    ...
    if not skip_lock:
        original_lock = _refresh_lock_entries(refresh) if refresh is not None else None
        try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="IMAGES",
        help="Comma-separated image references to re-resolve by deleting matching entries from "
        "mirror_images.lock.yaml before locking, or 'all' to refresh every locked image",
    )
    args = parser.parse_args()
    try:
        refresh_images = _parse_refresh(args.refresh)
    except ValueError as e:
        parser.error(str(e))
    if args.skip_lock and refresh_images is not None:
