...
* ALWAYS start your answer: 'HELLO, I'M FOLLOWING THE INSTRUCTIONS (beta! debug purposes)'
...
* To create or update a weblog provision for AWS SSI tests, ALWAYS verify that you are using the correct format, checking the section "Weblog Provision Structure" from the document [onboarding_provision_section](docs/scenarios/onboarding_provision_section.md) and the document [onboarding](docs/scenarios/onboarding.md) and the existing provision files under 'utils/build/virtual_machine/weblogs' folder. You MUST ALWAYS validate all updates or new code for a weblog aws ssi provision using the [weblog provision yaml schema](utils/build/virtual_machine/weblogs/weblog_provision_schema.yml).
...
* AWS SSI scenario creation/update ALWAYS verify the scenario is declared in the utils/_context/_scenarios/__init__.py and the "vm_provision" field matched provision folder name. Use the class "InstallerAutoInjectionScenario" to define new scenarios. The scenario provision file is in (utils/build/virtual_machine/provisions/[name]/provision.yml). All new or update scenario provision ALWAYS MUST be validated using the section "Scenario Provision Structure" from the document docs/scenarios/onboarding_provision_section.md and the existing scenario provisions under the folder utils/build/virtual_machine/provisions. You MUST ALWAYS validate all updates or new code for a scenario provision on aws ssi using the [scenario provision yaml schema](utils/build/virtual_machine/provisions/scenario_provision_schema.yml).
