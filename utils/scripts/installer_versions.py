...
def set_injector_version_from_lock() -> None:
    ...
    if os.getenv("DD_INSTALLER_LIBRARY_VERSION") and not os.getenv("DD_INSTALLER_INJECTOR_VERSION"):
        injector_version = AUTO_INJECT_LOCK.read_text(encoding="utf-8").strip()
        os.environ["DD_INSTALLER_INJECTOR_VERSION"] = injector_version
        logger.stdout(f"Using pinned injector version from {AUTO_INJECT_LOCK.name}: {injector_version}")
