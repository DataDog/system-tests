...
class TestAgentAPI:
    ...
    def wait_for_telemetry_configurations(
        ...
        *,
        ...
        validator: Callable[[dict[str, list[dict[str, Any]]]], bool] | None = None,
        clear: bool = False,
        wait_loops: int = 100,
        ...
        When provided, runtime_id filters events and validator defines the readiness condition.
        ...
        for _ in range(wait_loops):
            ...
            if events:
                ...
                if validator is None or validator(configurations):
                    if clear:
                        self.clear()
        ...
        raise AssertionError(f"No telemetry configurations matched the validator. Last observed: {configurations}")

    def wait_for_telemetry_runtime_id(self, *, exclude: set[str] | None = None, wait_loops: int = 200) -> str:
        excluded_runtime_ids = exclude or set()
        ...
        for _ in range(wait_loops):
            try:
                ...
                for event in events:
                    ...
                    if runtime_id not in excluded_runtime_ids:
                        return runtime_id
            time.sleep(0.01)
        raise AssertionError(
            f"No telemetry runtime ID found excluding {excluded_runtime_ids}. Observed: {observed_runtime_ids}"
        )
