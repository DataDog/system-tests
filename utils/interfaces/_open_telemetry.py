...
class OpenTelemetryInterfaceValidator(ProxyBasedInterfaceValidator):
                ...
                for scope_span in resource_span.get("scopeSpans"):
                    for span in scope_span.get("spans"):
                        for attribute in span.get("attributes", []):
                            ...
                            attr_val = attribute.get("value").get("string_value") or attribute.get("value").get("stringValue")
    ...
    def get_otel_spans(self, request: HttpResponse):
        ...
        for data in self.get_data(path_filters=paths):
            resource_spans = data.get("request").get("content").get("resource_spans") or data.get("request").get("content").get("resourceSpans")
    ...
    def get_trace_stats(self, resource: str):
        ...
        for data in self.get_data(path_filters=paths):
            resource_metrics = data.get("request").get("content").get("resource_metrics") or data.get("request").get("content").get("resourceMetrics")
                    ...
                    if scope_metric.get("scope").get("name") == "datadog.trace.metrics":
                        for metric in scope_metric.get("metrics"):
                            if metric.get("name") == "request.latencies":
                                data_points = metric.get("histogram").get("data_points") or metric.get("histogram").get("dataPoints")
