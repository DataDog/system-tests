        ...
        for t in self.get_telemetry_logs():
            ...
            if isinstance(payload, list):
                crash_reports.extend(
                    p for p in payload if "si_signo" in p.get("tags", "") or "signame" in p.get("tags", "") or "signum" in p.get("tags", "")
                )
            # If payload is a single object, check it directly
            elif isinstance(payload, dict):
                if "si_signo" in payload.get("tags", "") or "signame" in payload.get("tags", "") or "signum" in payload.get("tags", ""):
