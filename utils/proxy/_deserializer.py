...
def _uncompress_keys(trace_payload: dict):
...
def _uncompress_chunks(chunks: list):
    uncompressed_chunks = []
    for chunk in chunks:
        uncompressed_chunk = {}
...
def _uncompress_spans(spans: list):
    uncompressed_spans = []
    for span in spans:
        uncompressed_span = {}
...
def _unstream_strings(content: Any, strings: list[str] | None = None) -> list[str]:  # noqa: ANN401
    ...
    if content_type in ("application/msgpack", "application/msgpack, application/msgpack") or (path == "/v0.6/stats"):
        ...
        if source_is_datadog_tracer:
            if path == "/v0.4/traces":
                ...
                _unstream_strings(result)
                result = _uncompress_keys(result)
