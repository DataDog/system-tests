...
class Dev:
    ...
    def artifact_entries(
        ...
        return (text_entry("java-load-from-s3", resolved_inputs["library_branch"].sha),)


class Prod:
    def artifact_inputs(self, env: dict[str, str]) -> tuple[GitHubLatestReleaseResolver]:
        return (GitHubLatestReleaseResolver(name="release", repository="DataDog/dd-trace-java"),)

    def artifact_entries(
        self,
        resolved_inputs: dict[str, GitHubReleaseReference],
    ) -> tuple[ArtifactEntry]:
        return (text_entry("java-load-from-release", resolved_inputs["release"].tag_name),)
