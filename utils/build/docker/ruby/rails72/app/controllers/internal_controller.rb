class InternalController < ApplicationController
  ...
  def flush
    ...
    if telemetry = Datadog.send(:components)&.telemetry
      ...
      metrics_manager = worker.instance_variable_get(:@metrics_manager)

      # NOTE: In the current implementation there is no way to force the flushing.
      #       Instead we are checking if there are any pending events and flushing them
      #       manually.
      if worker.send(:work_pending?)
