...
const OPT_PROFILING_FILE = 'profiling-file';
const OPT_PROFILING_URL = 'profiling-url';
const OPT_PROFILING_VERSION = 'profiling-version';
...
function install_profiling($options, $selectedBinaries)
{

    echo "====================\n";
    echo "Installing profiling\n";

    $implicitProfiling = !isset($options[OPT_PROFILING_FILE]) &&
        !isset($options[OPT_PROFILING_URL]) &&
        !isset($options[OPT_PROFILING_VERSION]);

    if (
        $implicitProfiling ||
        isset($options[OPT_PROFILING_VERSION]) && $options[OPT_PROFILING_VERSION] == 'latest'
    ) {
        $options[OPT_PROFILING_VERSION] = latest_release("dd-prof-php");
    }

    // Preparing clean tmp folder to extract files
    $tmpDir = sys_get_temp_dir() . '/dd-profiling';
    $tmpDirTarGz = "$tmpDir/dd-profiling-php.tar.gz";
    execute_or_exit(
    ...
    execute_or_exit(
        "Cannot clean '$tmpDir'",
        "rm -f " . escapeshellarg($tmpDirTarGz)
    );
    delete_on_exit($tmpDir);

    // Retrieve the archive to the temporary location
    if (isset($options[OPT_PROFILING_FILE])) {
        $tmpDirTarGz = $options[OPT_PROFILING_FILE];
    } else {
        $url = isset($options[OPT_PROFILING_URL])
            ? $options[OPT_PROFILING_URL]
            : sprintf(
                'https://github.com/DataDog/dd-prof-php/releases/' .
                'download/v%1$s/datadog-profiling.tar.gz',
                rawurlencode($options[OPT_PROFILING_VERSION])
            );
        download($url, $tmpDirTarGz);
    }
...
function parse_validate_user_options()
{
    ...
    if (!$normalizedOptions[OPT_UNINSTALL]) {
        ...
        $installables = array_intersect(
            [OPT_PROFILING_VERSION, OPT_PROFILING_URL, OPT_PROFILING_FILE],
            array_keys($options)
        );
        if (count($installables) > 1) {
            print_error_and_exit(
                'Only one among --profiling-version, --profiling-url and --profiling-file must be provided'
            );
            }
