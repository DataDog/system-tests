...
if php-config --prefix | grep -q release-zts; then
    sed -i "s/%MPM/event/" /etc/apache2/mods-available/php.load
else
    sed -i "s/%MPM/prefork/" /etc/apache2/mods-available/php.load
fi

if ! { echo $VARIANT | grep -q zts; }; then
    a2dismod mpm_event
    a2enmod mpm_prefork
fi
