...
func main() {
	...
	// echotrace.Wrap installs the tracing middleware, the OnAddRoute hook for
	// API Catalog, and the AppSec request-body Binder wrap in one call.
	r := echotrace.Wrap(echo.New())
	...
	// StartConfig.Start: cancelling the context triggers a graceful shutdown.
	ctx, cancel := signal.NotifyContext(context.Background(), syscall.SIGTERM)
	defer cancel()
	sc := echo.StartConfig{Address: ":7777"}
	if err := sc.Start(ctx, r); err != nil && !errors.Is(err, http.ErrServerClosed) {
		logrus.Fatal(err)
	}
}
...
func echoHandleFunc(handlerFunc http.HandlerFunc) echo.HandlerFunc {
	return func(c *echo.Context) error {
		// v5: c.Response() returns http.ResponseWriter directly (no .Writer field).
...
func headers(c *echo.Context) error {
	...
	c.Response().Header().Set("content-type", "text/plain")
	c.Response().Header().Set("content-length", "42")
	c.Response().Header().Set("content-language", "en-US")
