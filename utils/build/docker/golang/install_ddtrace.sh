...
if [ -e "/binaries/dd-trace-go" ]; then
	...
	for line in "${lines[@]}"; do
		...
		GO_GET_VERSION=$(go list -m -json "$path@$commit" | jq -r .Version)
		go mod edit -replace "$path=$path@$GO_GET_VERSION"
		break
	done
else
...
# Run go mod tidy once after the main-module replacement. This lets feature
# branches add packages before they exist in the released module.
go mod tidy
...
if [ -e "/binaries/golang-load-from-go-get" ]; then
	for contrib in $CONTRIBS; do
		echo "Install contrib $contrib from go get -v $contrib@$GO_GET_VERSION"
		go mod edit -replace "$contrib=$contrib@$GO_GET_VERSION"
	done
else
	for contrib in $CONTRIBS; do
