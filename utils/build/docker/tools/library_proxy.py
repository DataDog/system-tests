...
if __name__ == '__main__':
    ...
    # warmup the cache and check build output
    for msg in build_image_directly():
        parse_output(msg)
    ...
    iterations = 10
    time = timeit.timeit(lambda: build_image_directly(), number = iterations ) / iterations
    print("time per iteration: ", time)

    print("timing the docker build that isolates dependencies")
    iterations = 100
    time = timeit.timeit(lambda: build_image_indirectly(), number = iterations ) / iterations
    print("time per iteration: ", time)
