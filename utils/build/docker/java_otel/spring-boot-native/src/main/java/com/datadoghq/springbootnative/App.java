...
public class App {
    ...
    private static SdkTracerProvider setupTraceProvider(Resource resource) {
        ...
        if (isAgentEnabled()) {
            spanExporters.add(
                OtlpHttpSpanExporter.builder()
                    .setEndpoint("http://proxy:9001/v1/traces")
        ...
        if (isIntakeEnabled()) {
            spanExporters.add(
                OtlpHttpSpanExporter.builder()
                    .setEndpoint("http://proxy:9001/api/v0.2/traces")  // send to the proxy first
                    .addHeader("dd-protocol", "otlp")
    ...
    private static SdkMeterProvider setupMeterProvider(Resource resource) {
        ...
        if (isAgentEnabled()) {
            metricExporters.add(
                    OtlpHttpMetricExporter.builder()
                            .setEndpoint("http://proxy:9001/v1/metrics")
                            .addHeader("dd-protocol", "otlp")
                            .addHeader("dd-otlp-path", "agent")
        ...
        if (isIntakeEnabled()) {
            metricExporters.add(
                    OtlpHttpMetricExporter.builder()
                            .setEndpoint("http://proxy:9001/api/intake/otlp/v1/metrics")  // send to the proxy first
                            .addHeader("dd-protocol", "otlp")
                            .addHeader("dd-api-key", System.getenv("DD_API_KEY"))
    ...
    private static SdkLoggerProvider setupLoggerProvider(Resource resource) {
        ...
        if (isAgentEnabled()) {
            logRecordExporters.add(
                    OtlpHttpLogRecordExporter.builder()
                            .setEndpoint("http://proxy:9001/v1/logs")
                            .addHeader("dd-protocol", "otlp")
                            .addHeader("dd-otlp-path", "agent")
                            .build());
        }
        if (isIntakeEnabled()) {
            logRecordExporters.add(
                    OtlpHttpLogRecordExporter.builder()
                            .setEndpoint("http://proxy:9001/api/v2/logs")
                            .addHeader("dd-protocol", "otlp")
                            .addHeader("dd-api-key", System.getenv("DD_API_KEY"))
