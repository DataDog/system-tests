...
template <typename T> std::optional<T> get_if_exists(const nlohmann::json& j, std::string_view key) {
...
public:
  ...
  dd::Optional<dd::StringView> lookup(dd::StringView key) const override {
    for (const auto& subarray : headers_) {
      if (subarray.size() == 2) {
        ...
        } else if (tolower(subarray[1].get<std::string>()) == std::string(key)) {
  ...
  void visit(const std::function<void(dd::StringView key, dd::StringView value)>& /* visitor */) const override {}
};
...
public:
  ...
  // `key`.
  void set(dd::StringView key, dd::StringView value) override { j_.emplace_back(nlohmann::json::array({key, value})); };
};
