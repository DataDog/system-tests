...
impl Baggage {
    ...
    fn get_entry_value(&self, key: &str) -> Option<String> {
...
#[derive(Clone)]
struct AppState {
    spans: Arc<Mutex<HashMap<u64, BoxedSpan>>>,
    baggage: Arc<Mutex<Baggage>>,
...
pub fn app(tracer_provider: Arc<SdkTracerProvider>) -> Router {
    let state = AppState {
        ...
        baggage: Arc::new(Mutex::new(Baggage::empty())),
        tracer_provider,
    };
...
async fn start_span(
    ...
    if let Some(links) = args.links {
        ...
        for link in links {
            if let Some(span) = state.spans.lock().unwrap().get(&link.parent_id) {
...
async fn get_span_context(
    ...
    let result = if let Some(span) = state.spans.lock().unwrap().get(&args.span_id) {
...
async fn is_recording(
    ...
    let recording = if let Some(span) = state.spans.lock().unwrap().get(&args.span_id) {
...
async fn set_name(State(state): State<AppState>, Json(args): Json<SetNameArgs>) {
    if let Some(span) = state.spans.lock().unwrap().get_mut(&args.span_id) {
...
async fn set_status(State(state): State<AppState>, Json(args): Json<SetStatusArgs>) {
    let span_id = args.span_id;
    if let Some(span) = state.spans.lock().unwrap().get_mut(&span_id) {
