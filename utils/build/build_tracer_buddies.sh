...
docker buildx build --load --progress=plain -f utils/build/docker/python/flask-poc.Dockerfile -t datadog/system-tests:python_buddy-v2 .
...
if [ "$1" = "--push" ]; then
      docker push datadog/system-tests:python_buddy-v2
