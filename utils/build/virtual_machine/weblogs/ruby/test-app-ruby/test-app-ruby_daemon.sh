...
do_start(){
    start-stop-daemon --start $common_opts --chdir $DIR --make-pidfile --background --startas /bin/bash -- -c "DD_APM_INSTRUMENTATION_DEBUG=true DD_APM_INSTRUMENTATION_OUTPUT_PATHS=/shared_volume/std.out rails server -b 0.0.0.0 -p 5985 >> $LOG 2>&1"
}
