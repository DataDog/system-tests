...
do_start(){
    start-stop-daemon --start $common_opts --chdir $DIR --make-pidfile --background --startas /bin/bash -- -c "DD_APM_INSTRUMENTATION_DEBUG=true DD_APM_INSTRUMENTATION_OUTPUT_PATHS=/shared_volume/std.out node /home/datadog/index.js >> $LOG 2>&1"
}
