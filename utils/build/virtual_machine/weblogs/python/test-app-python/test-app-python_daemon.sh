...
do_start(){
    start-stop-daemon --start $common_opts --chdir $DIR --make-pidfile --background --startas /bin/bash -- -c "DD_APM_INSTRUMENTATION_DEBUG=true DD_APM_INSTRUMENTATION_OUTPUT_PATHS=/shared_volume/std.out PYTHONUNBUFFERED=1 DJANGO_SETTINGS_MODULE=django_app /home/datadog/.pyenv/shims/python -m django runserver 0.0.0.0:5985 >> $LOG 2>&1"
}
