...
* Use the document README.md to provide a good answer about a quick overview of the system-tests or how to run them for the first time.
...
* All references to documentation files, scripts, or source code in this instruction/rules file start from the workspace root. Files are referenced directly, through markdown links, or by putting an @ at the beginning of the reference path.
...
* All YAML files must be formatted using `yamllint` or `yamlfmt`.
...
* You always need a context to provide a good answer. You need to know if the user is talking about a specific type of scenario: end-to-end, parametric, Docker SSI, K8s lib injection, or AWS SSI tests. Ask the user if you don't know the context.
