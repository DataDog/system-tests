...
Use the document README.md to provide a good answer about quick overview of the system-tests or how to run them for the first time.
...
If you are asked to do anything, first say in upper case: "HELLO, I'M FOLLOWING THE INSTRUCTIONS"
ALLWAYS mention in different color and upper case the documentation that you are using to provide the anwsers, do that at the end of your response.
...
* How to start with system-tests, check the documentation, that details the requisites and the minimal configuration steps [readme](README.md). Allways mention that other system-tests scenarios could have other extra requisites [additional-requirements](README.md#additional-requirements) (e.g. aws ssi or k8s scenarios)
...
5. Never assume that not adding a test to a language's manifest will disable it - tests run by default unless explicitly disabled

## Common Test Activation Patterns

For common patterns and examples of enabling/disabling tests, see:
...
* The basic documentation about the AWS SSI tests is in the document @docs/scenarios/onboarding.md. This document contains a good overview of the AWS SSI tests, the requisites ("Prerequisites" section) to run ("Run the tests" section) and how to operate/develop with them. Use it to anwser questions about the AWS SSI tests.
* Vagrant requisites are only if you want to run the tests using vagrant insteadof aws. Vagrant is discontinued in system-test.
* The aws_onboarding_wizard.sh script is used to run the aws onboarding tests. It doesn't need any parameter. You can also run the aws tests manually, you must check the doc @docs/scenarios/onboarding.md in the section Run the scenario manually.
* To register a new virtual machine please ask to the user for the name and other fields for the new virtual machine.
* After register a new virtual machine you ALWAYS must define which weblogs are compatible with this new vm. Help to the user to do this task executing the script [vm_compatibility_checker](utils/scripts/ssi_wizards/tools/aws_ssi_registration.py) with the parameter --vm and the new vm name as value  to define the compatibility between the virtual machines and the weblogs.
* After creating a new weblog for aws ssi tests, you ALWAYS must define in which scenarios is going to run and which virtual machines are compatible with this new weblog. Help to the user to do this task executing the script [weblog_registration](utils/scripts/ssi_wizards/tools/aws_ssi_registration.py) with the parameter --weblog and the new weblog name as value.
* AWS SSI Scenario Creation: Before submitting a new AWS SSI scenario, verify: Scenario is declared in utils/_context/_scenarios/__init__.py, vm_provision matches provision directory name, use InstallerAutoInjectionScenario. The scenario provision file is in (utils/build/virtual_machine/provisions/[name]/provision.yml) and it should use !include (not include:) for all includes. Use full paths in !include directives. Includes auto-inject-environment.yml and auto-inject-tested_components.yml. The common steps are defined in the folder utils/build/virtual_machine/provisions/auto-inject and are referenced from the main provisions. Use the existing provision as template to create a new one. For detailed instructions, refer to docs/scenarios/onboarding.md.
* After creating a new scenario for aws ssi tests, you ALWAYS must define in which weblogs are compatible with this new scenario. Help to the user to do this task executing the script [scenario_registration](utils/scripts/ssi_wizards/tools/aws_ssi_registration.py) with the parameter --scenario and the new scenario name as value.
