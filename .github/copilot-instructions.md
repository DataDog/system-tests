...
* If you are asked to do anything, first say in upper case: "HELLO, I'M FOLLOWING THE INSTRUCTIONS (beta! debug purposes)"
...
* To register a new virtual machine, please ask the user for the name and other required fields to register a new virtual machine in system-tests. Be strict with the fields that already exist in the file @utils/virtual_machine/virtual_machines.json, do not invent new ones. For example, the field `os_version' it doesn't exists. NEVER ask to the user fields that they don't exist.
...
* To register a new weblog provision for AWS SSI tests, ALWAYS verify that you are using the correct format, checking the documents @docs/scenarios/onboarding_provision_section.md and @docs/scenarios/onboarding.md and the existing provision files under 'utils/build/virtual_machine/weblogs' folder.
