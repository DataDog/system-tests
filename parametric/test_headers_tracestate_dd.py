...
def test_tracestate_dd_propagate_samplingpriority(test_agent, test_library):
    ...
    with test_library:
        ...
        headers2 = make_single_request_and_get_headers(test_library, [
            ['x-datadog-trace-id', '7890123456789012'],
            ['x-datadog-parent-id', '1234567890123456'],
        ...
        headers3 = make_single_request_and_get_headers(test_library, [
            ['traceparent', '00-12345678901234567890123456789012-1234567890123456-01'],
        ])

        # 4) Sampled = 1, tracestate[dd][s] <= 0
        headers4 = make_single_request_and_get_headers(test_library, [
            ['traceparent', '00-12345678901234567890123456789012-1234567890123456-01'],
            ['tracestate', 'foo=1,dd=s:-1'],
        ])

        # 5) Sampled = 1, tracestate[dd][s] > 0
        headers5 = make_single_request_and_get_headers(test_library, [
            ['traceparent', '00-12345678901234567890123456789012-1234567890123456-01'],
            ['tracestate', 'foo=1,dd=s:2'],
        ])

        # 6) Sampled = 0, tracestate[dd][s] is not present
        headers6 = make_single_request_and_get_headers(test_library, [
            ['traceparent', '00-12345678901234567890123456789012-1234567890123456-00'],
        ])

        # 7) Sampled = 0, tracestate[dd][s] <= 0
        headers7 = make_single_request_and_get_headers(test_library, [
            ['traceparent', '00-12345678901234567890123456789012-1234567890123456-00'],
            ['tracestate', 'foo=1,dd=s:-1'],
        ])

        # 8) Sampled = 0, tracestate[dd][s] > 0
        headers8 = make_single_request_and_get_headers(test_library, [
            ['traceparent', '00-12345678901234567890123456789012-1234567890123456-00'],
            ['tracestate', 'foo=1,dd=s:1'],
        ])
...
def test_tracestate_dd_propagate_origin(test_agent, test_library):
    ...
    assert 'traceparent' in headers3
    assert 'tracestate' in headers3
    assert 'o:synthetics__web' in dd_items3
