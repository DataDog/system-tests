...
We'll implement a mechanism that will put metadata on each test class about the coverage/completeness of the test. Two questions :
...
- The test is written. It provides a certain level of guarantee for the feature to be bug-free
...
- indirect testing, it guess that the feature is working because another one is working
- a simple smoke test
- a quite complete test
- a rock solid test, implying a very strong confidence on the feature

The second state is more ambiguous. Often, the test class is missing, but we don't know if it's because the feature is not testable or if we can one day implement it.
...
System test outputs are exported and used in metabase or google spreadsheets. As some feature are not testable, those UI needs to mix declarative data, and system test output. As an declarative test class is not harder than 2 code line, having this status will help us to have a complete automatic UI for feature status.
...
The test is the most simple test we can imagine for a feature. It ensure that the feature is working, but does not cover any subtlety or edge cases

### Good test

The test fully covers the core functionality of the feature. The test will catch all bug that can happen, except for edge cases and extreme scenarios

### Complete test

The test fully cover the functionality, and missing a bug is statistically negligible => If a bug is missed, we should be included in the post mortem and review our understanding of the feature
...
```python
@coverage.not_testable  # DEPRECATED: this information exists in the feature parity dashboard
class Test_Feature():
    pass

@coverage.not_implemented  # DEPRECATED: this information exists in the feature parity dashboard
class Test_Feature():
    pass
...
The test coverage will be used to have action items on test implementations. In consequence, it must be unique for a given test class. If more than one is used, an exception is raised
...
In particular, weblog missing endpoints can't have impact on test coverage, as there is nothing to do on the test class. We must use `missing_feature` for this use case (yes, it is a missing feature, _test is a true part of the feature_) :
...
### Other implementation : Using base test class inheritance

This implementation has not been chosen because :

- it would make the declaration mandatory => causes friction
...
DEPRECATED: this information exists in the feature parity dashboard
