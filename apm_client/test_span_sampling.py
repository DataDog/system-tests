...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver","name":"web.request"}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webse*","name":"web.re?uest"}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"notmatching","name":"notmatching"}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver","name":"web.request"}, {"service":"webserver","name":"web.request", "sample_rate":0}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver","name":"web.request", "sample_rate":0}, {"service":"webserver","name":"web.request"}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver","name":"web.request", "sample_rate":1}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver", "name":"web.request"}]), "DD_TRACE_STATS_COMPUTATION_ENABLED":"True"}])
def test_keep_span_with_stats_computation_sss010(test_agent, test_client: _TestTracer):
    ...
    assert os.getenv("DD_TRACE_STATS_COMPUTATION_ENABLED") == True
    generate_span(test_client)
    span = get_span(test_agent)
...
def test_single_rule_always_keep_span_sampling_sss011(test_agent, test_client: _TestTracer):
    """Test spans are always kept when sampling rule matches and has sample_rate:1.0"""
    generate_span(test_client)
    span = get_span(test_agent)
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver","name":"web.request", "sample_rate":0}])}])
...
@pytest.mark.parametrize("apm_test_server_env", [{"DD_SPAN_SAMPLING_RULES":json.dumps([{"service":"webserver","name":"web.request", "max_per_second":1}, {"service":"webserver2","name":"web.request2", "max_per_second":5}])}])
