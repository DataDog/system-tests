    ...
    def captured_spans(self):
        ...
        for span in self._order:
            ...
            for l in (span_links or []):
                ...
                links.append(CapturedLink(str(l.span_id), _low64(l.trace_id), int(l.trace_id >> 64), attrs))
    ...
    def _coerce_event_attrs(self, attrs):
        def scalar(kind, v):
            ...
            if kind == "bool":
                return v == "true"
            return v
        # intentionally-invalid inputs (for the discard test); the tracer should drop these
        invalid = {
            "raw_mixed_arr": [1, "a"],
            "raw_nested_arr": [[1]],
            "raw_huge_int": 2 ** 66,
        ...
        out = {}
        for a in attrs:
            if a.kind in invalid:
                out[a.key] = invalid[a.kind]
                continue
            vals = list(a.values)
