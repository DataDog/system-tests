...
// Synchronous HTTP GET (curl, no shell) — the Temper run functions are
// synchronous and can't drive node's async fetch/event loop.
function curlGet (url) {
  try {
    const r = spawnSync('curl', ['-s', url], { encoding: 'utf8', timeout: 4000 })
...
export function makeAdapter (tracer, otel = otelApi) {
  ...
  const v04encoder = V04Encoder ? new V04Encoder(8 << 20, 8 << 20) : encoder
  ...
  class DdTraceAdapter extends Tracer {
    ...
    rcCapabilitiesCsv () {
      ...
      try { spawnSync(process.execPath, [path.join(here, 'rc-poll.mjs')], { stdio: 'ignore', timeout: 9000 }) } catch {}
      try {
        const reqs = JSON.parse(curlGet(`${base}/test/session/requests`) || '[]')
        for (const r of reqs) {
    ...
    computedStatsJson () {
      ...
      while (Date.now() < deadline) {
        let stats = []
        try { stats = JSON.parse(curlGet(`${base}/test/session/stats`) || '[]') } catch {}
    ...
    wireSpanEventsJson (spanId) {
      ...
      if (!f || !v04encoder || !msgpackDecode) return ''
      try {
        v04encoder.reset()
        v04encoder.encode([f])
        const buf = v04encoder.makePayload()
    ...
    capturedSpans () {
      return this.#formatted.map((f) => {
        ...
        const links = (live?._links ?? []).map((l) => {
          ...
          // high 64 bits of a 128-bit trace id, as an unsigned decimal string
          // (a full W3C trace-id-high overflows Number/Int precision); '0' if absent
          const tidHex = l.context?._trace?.tags?.['_dd.p.tid']
          const high = tidHex ? BigInt('0x' + tidHex).toString() : '0'
...
let V04Encoder = null
try { V04Encoder = require('dd-trace/packages/dd-trace/src/encode/0.4.js').AgentEncoder } catch {}
let V05Encoder = null
